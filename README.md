...
カテゴリごとに別々の mixi2 Plugin アプリケーションアカウントを使い、同じコミュニティへカテゴリタグ付きで投稿します。
...
mixi2 Developer Platform でカテゴリごとに Plugin アプリケーションを作成し、投稿先コミュニティにインストールします。この Plugin は arXiv から取得した内容をコミュニティに投稿するだけなので、イベント受信用の Webhook URL や Stream Address は使いません。
...
7. Client ID と Client Secret を控えます。Token URL、API Address、Community ID は共通の値として控えます。
